...
int main() {
    ...
    if ((cached = fopen(cacheName, "r")) != NULL) { 
        ...
        //converte epoch in struct tm
        struct tm tmSaved = *gmtime(&epochSaved);
        char httpDateCache[30];

        //converte struct tm in stringa http date
        strftime(httpDateCache, sizeof(httpDateCache), format, &tmSaved);
        printf("Last time of download is %s\n", httpDateCache);
        
        /// 1) request HEAD for the resource: client chiede al server se la risorsa è stata modificata dall'ultimo download
        ...
        // read the Last-Modified header. CONTROLLO STATUS CODE
        int bodyLength = 0;
        char* lastModified = 0;
        int notModified = 0;
        if (!strcmp("304 Not Modified", headers[0].n+9)) { //il +9 salta i primi 9 caratteri e punta direttamente a 304
        ...
        //ciclo for inutile, serve solo per stampare gli header della risposta alla richiesta HEAD
        for (int i=1; i<headersCount; i++)
            if (!strcmp("Last-Modified", headers[i].n))
                lastModified = headers[i].v;  //se non è stata modificata, last modified è zero
        ...
        if (notModified || (epochRemote <= epochSaved)) {  //se 0/1 (falso) e Vero allora entro nel if
            printf("\n----->Serve from cache!!!");
            int c=0;
    ...
    /// 3.a) download resource. GET senza if modified since, perche vogliamo il contenuto completo
    sprintf(request, "GET %s HTTP/1.1\r\nHost:%s\r\n\r\n", resourceName, hostname);
    write(s, request, strlen(request));
    
    int headersCount = 0;

    //pulisce buffer prima di usarli
