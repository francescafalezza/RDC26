...
//struttura che descrive un indirizzo IPv4 remoto
struct sockaddr_in remote;

//buffer per la richiesta da inviare al server e per la risposta da ricevere
char request[1024];
...
int main() {
    ...
    strcat(cacheName, tmp); //funzione per unire due stringhe
    ...
    //Costrusce richiesta HTTP HEAD, cioè il server risponde solo con gli header, senza body
    ...
    /*Tenta di aprire un file in lettura, se il file non esiste fopen ritorna NULL*/
    if ((cached = fopen(cacheName, "r")) != NULL) { 
        printf("\n----->Resource already downloaded, verify cache\n");
        
        //invia richiesta al server sul socket s
        write(s, request, strlen(request));

        //parsing degli header della risposta del server
        int headersCount = 0;
        headers[0].n = hResponse; //primo header inizia all'inizio del buffer hResponce
        ...
        char* format = "%a, %d %b %Y %T GMT";     // man 3 strptime. Formato: giorno, giorno settimana, mese, anno 4 cifre, hh:mm:ss
        struct tm* httpTime = malloc(sizeof(struct tm));
        strptime(lastModified, format, httpTime); //analogo di scanf per le date
        time_t epochRemote = mktime(httpTime);  //converte la struct tm in un valore time_t, cioè in secondi a partire dal 1/1/1970
        ...
        char cacheTime[10];
        fread(cacheTime, sizeof(char), 10, cached); //legge 10 bytes dal file cached e li mette in cacheTime
        ...
        if (epochRemote <= epochSaved) {
            ...
            //legge il file di cache a blocchi di 5KB
    ...
    /*HTTP1.0 chiude la connessione dopo ogni risposta, quindi quando si arriva al GET il socket è già chiuso
    Bisogna aprire una nuova connessione*/
    //Cache scaduta o file non esiste, viene fatta richiesta GET
    /// 3) download the resource and save it in dir ./cache/    

    //Viene inviata una richiesta GET al server per ottenere il body

    printf("\n----->Resource need to be downloaded\n");
    char* lastModified = 0;
    

    // chiudi il vecchio socket (usato per la HEAD)
    close(s);

    // crea un nuovo socket
    if (-1 == (s = socket(AF_INET, SOCK_STREAM, 0))) {
        perror("Socket fallita"); return -1;
    }
    ...
    for (int i=0; read(s, hResponse + i, 1); i++) { //a ogni iterazione legge un byte dal socket e lo mette in hresponse
        if (hResponse[i] == '\n' && hResponse[i-1] == '\r') {
            hResponse[i-1] = 0;         // Terminate token
            // If the previous header's key name is all 0s, we reached the end of the headers section.
            if (!headers[headersCount].n[0]) break;
    ...
    time_t epochNow = time(NULL); //ritorna i secondi attuali dall'epoch
    fprintf(cached, "%lu\n", (unsigned long)epochNow); //viene scritto come numero intero nella prima riga del file cache
