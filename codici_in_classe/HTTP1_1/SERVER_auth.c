 ...
 *
*/
//Base64 converte ogni 3 byte di input in 4 caratteri
void f(unsigned char *a, char *d){
	d[0] = base64[a[0] >> 2];  //prende 6 bit più significativi del primo byte e li usa come indice nell'afabeto Base64
...
void base64_encode(unsigned char *in, int in_len, char *out){
	...
	for(i=0; i<in_len-2; i+=3){
		f(in + i, out + out_offset);  //processa 4 byte alla volta
		out_offset += 4;            //produce 4 caratteri Base64
	}
	if(in_len % 3 == 2){
		unsigned char x[3];
		x[0] = in[i];
...
int main() {
    ...
    while (1){
        ...
        //qyando arriva una richiesta crea un nuovo socket dedicato a quella connessione, s continua ad ascoltare
        int s2 = accept(s, (struct sockaddr *)&remote, &len);
        bzero(hRequest, buffLen); //azzera buffer della richiesta
        ...
        // read method, url and version. PARSING DELLA REQUEST LINE
        char * method, *url, *ver;  
        int p = 0;  // p act as offset
        // read method
        method = reqLine;
        // read url. Scorre fino allo spazio, metto /0 per terminare method, poi url punta al carattere successivo
        for(p = 0; reqLine[p] != ' '; p++);
        reqLine[p++] = 0; 
        url = reqLine + p;
        ...
        if (!strcmp(method, "GET")){
            ...
            // file found, write it in response
                ...
                //se l'url inizia con /secure/ scatta il controllo autenticazione
                if (!strncmp(url, "/secure/", strlen("/secure/"))) {
                    char* authHeader = 0;
                    int i = 0;

                    //cerca nell'array degli header uno che si chiami "Authorization" e salva il suo valore in authHeader
                    ...
                    if (authHeader)
                        userFound = -1 < matchUser(authHeader+strlen("Basic ")); //risultato è 1 se l'utente esiste
                    ...
                    //Condizione vera se almeno uno dei tre è vero. 
                    if (!authHeader || strncmp(authHeader, " Basic", strlen(" Basic")) || !userFound) {
                        sprintf(response, "HTTP/1.1 401 Unauthorized\r\nWWW-Authenticate:basic\r\n\r\n");
                        write(s2, response, strlen(response));
                        printf("-- End of request\n\n");
                        fclose(fin);
                        close(s2); //CHIUSA CONNESSIONE, SI RIPARTE DAL WHILE
                        continue;
                    }
                }
                ...
                // if file is bigger than 5 KB. Per file grandi usa chunked, non serve conoscere dimensione totale in anticipo
                if (size > (1024 * 5)) {
                    ...
                    /*FUNZIONE FREAD: 
                    fileBuff → buffer dove scrivere i dati letti
                    sizeof(char) → dimensione di ogni elemento = 1 byte
                    1024*5 → quanti elementi leggere = 5120 byte max
                    fin → il file da cui leggere
                    ritorna → quanti byte ha effettivamente letto (≤ 5120)*/
...
/*Client Browser
     │
     │ TCP connect porta 17999
     ▼
  accept() ──► nuovo socket s2
     │
     │ legge richiesta HTTP byte per byte
     ▼
  parsing headers
     │
     ├─ URL inizia con /secure/ ?
     │       │
     │       ├─ NO  → serve il file direttamente
     │       │
     │       └─ SÌ  → c'è header Authorization?
     │                    │
     │                    ├─ NO  → 401 + WWW-Authenticate (browser mostra popup)
     │                    │
     │                    └─ SÌ  → matchUser() controlla passwd.txt
     │                                 │
     │                                 ├─ credenziali OK → serve il file
     │                                 └─ credenziali KO → 401
     │
  invia file (chunked se >5KB, Content-Length se ≤5KB)
     │
  close(s2)*/