...
E' un proxy quindi posso controllare e modificare richieste e risposte solo se il metodo dell richiesta è GET.

Implementazione: 
Quando arriva una richiesta dal CLIENT controllo l'header Host e il suo valore (risoluzione host-->indirizzo IP)

    - se l'indirizzo IP del server è uno di quelli nella lista block allora può andare avanti la richiesta
    - altrimenti viene mandato un messaggio di errore al client. 
...
int main() {
    ...
    //lista block con indirizzi IP autorizzati. 
    
    char *safe[]={"1.1.1.1", "88.80.187.84", "www.google.com", "www.unipd.com" };
  ...
  // Ovvero max 10 tentativi diversi di accedere alla risorsa
    ...
    // dal resto che viene messo in vet
		...
		if(!strcmp("GET", method)){
            ...
            hostToConfront= inet_ntoa(*(struct in_addr*)(he->h_addr_list[0]));

            //----------------------------------PARTE NUOVA--------------------
            int inList=0;
            for(int i =0; i<4; i++){
                struct hostent *heSafe = gethostbyname(safe[i]);

                safeIP= inet_ntoa(*(struct in_addr*)(heSafe->h_addr_list[0]));

                if(strcmp(safe[i], hostToConfront)==0){
                        inList=1; 
                        printf("Indirizzo IP %s del server è nella lista safe", hostToConfront);
                }
            }
            ...
            //------------------------FINE PARTE NUOVA-------------------------
            //ERRORE: bisogna gestire gli indirizzi di google e unipd e risolverli in inidirizzi ip
